...
os.makedirs("images_hem", exist_ok=True)
...
dataset_path = 'leukemia-classification/C-NMC_Leukemia_restructure/training_data/hem/'
...
for epoch in range(opt.n_epochs):
    for i, (imgs, _) in enumerate(dataloader):
        ...
        if batches_done % opt.sample_interval == 0:
            save_image(gen_imgs.data[:25], "images_hem/%d.png" % batches_done, nrow=5, normalize=True)

        if (epoch + 1) % 20 == 0 and i == len(dataloader) - 1:  # Adding 1 because epoch starts from 0
            torch.save(generator.state_dict(), f'generator_weights_hem_epoch_{epoch+1}.pth')
            torch.save(discriminator.state_dict(), f'discriminator_weights_hem_epoch_{epoch+1}.pth')
...
plt.ylabel("Loss")
plt.legend()
plt.savefig("training_loss_HEM_plot.png")
...
save_dir = "images_result_gan_hem"
